...
#define  MAXGALFAC        1
#define  ABSOLUTEMAXSNAPS 200        /* Maximum number of snapshots supported for any simulation */
...
#define  MAX_STRING_LEN   1024  /* Max length of a path or name string */
...
/* Compile-time assertion. Uses a typedef rather than a bitfield struct (the
   Linux kernel approach) to avoid an unused-value warning under -Wall. The
   corresponding -Wno-unused-local-typedefs flag is set in the Makefile. */
...
#define ABORT(sigterm)                                                                               \
    do {                                                                                             \
        ...
        fprintf(stderr, "If the fix to this isn't obvious, please feel free to open an issue at:\n" \
...
#ifdef NDEBUG
#define XASSERT(EXP, EXIT_STATUS, ...) do { } while(0)
#else
