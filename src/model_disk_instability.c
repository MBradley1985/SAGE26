                            ...
                            struct GALAXY *galaxies, struct params *run_params)
{
    ...
    if(diskmass > 0.0) {
        ...
        if(unstable_gas > 0.0 && galaxies[p].ColdGas > 0.0) {
            ...
            const double initial_cold_gas = galaxies[p].ColdGas;
            ...
            // CALCULATE LEFTOVER UNSTABLE GAS
            const double gas_consumed = initial_cold_gas - galaxies[p].ColdGas;
            const double leftover_unstable = unstable_gas - gas_consumed;
            
            // If the starburst didn't consume all unstable gas, it must sink to the bulge
            if(leftover_unstable > 0.0 && galaxies[p].ColdGas > 0.0) {
                const double metallicity = get_metallicity(galaxies[p].ColdGas, galaxies[p].MetalsColdGas);
                
                galaxies[p].ColdGas -= leftover_unstable;
                galaxies[p].MetalsColdGas -= metallicity * leftover_unstable;
                
                galaxies[p].StellarMass += leftover_unstable;
                galaxies[p].BulgeMass += leftover_unstable;
                galaxies[p].InstabilityBulgeMass += leftover_unstable;
                
                galaxies[p].MetalsStellarMass += metallicity * leftover_unstable;
                galaxies[p].MetalsBulgeMass += metallicity * leftover_unstable;
                
                if(run_params->DustOn == 1) {
                    const double DTG = get_DTG(galaxies[p].ColdGas + leftover_unstable, galaxies[p].ColdDust);
                    galaxies[p].ColdDust -= DTG * leftover_unstable;
