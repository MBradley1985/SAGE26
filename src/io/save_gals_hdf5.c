...
int32_t initialize_hdf5_galaxy_files(const int filenr, struct save_info *save_info, const struct params *run_params)
{
    ...
    for(int32_t snap_idx = 0; snap_idx < run_params->NumSnapOutputs; snap_idx++) {
        ...
        // Create 2D DustDot datasets (DustDotForm, DustDotGrowth, DustDotDestruct)
        // Dimensions: [ngal (unlimited), STEPS (fixed)]
        {
            const char *dustdot_names[3] = {"DustDotForm", "DustDotGrowth", "DustDotDestruct"};
            const char *dustdot_desc[3] = {
                "Dust formation rate from stellar sources (per step within snapshot). Units: Msun/yr",
                "Dust grain growth rate by metal accretion (per step within snapshot). Units: Msun/yr",
                "Dust destruction rate by SN shocks (per step within snapshot). Units: Msun/yr"
            };

            hsize_t dd_dims[2] = {0, (hsize_t)STEPS};
            hsize_t dd_maxdims[2] = {H5S_UNLIMITED, (hsize_t)STEPS};
            hsize_t dd_chunk_dims[2] = {NUM_GALS_PER_BUFFER, (hsize_t)STEPS};

            for(int dd_idx = 0; dd_idx < 3; dd_idx++) {
                snprintf(full_field_name, 2*MAX_STRING_LEN - 1, "Snap_%d/%s", run_params->ListOutputSnaps[snap_idx], dustdot_names[dd_idx]);

                hid_t dd_prop = H5Pcreate(H5P_DATASET_CREATE);
                CHECK_STATUS_AND_RETURN_ON_FAIL(dd_prop, (int32_t) dd_prop,
                                                "Could not create property list for %s dataset at snapshot %d.\n", dustdot_names[dd_idx], snap_idx);

                hid_t dd_dataspace = H5Screate_simple(2, dd_dims, dd_maxdims);
                CHECK_STATUS_AND_RETURN_ON_FAIL(dd_dataspace, (int32_t) dd_dataspace,
                                                "Could not create 2D dataspace for %s at snapshot %d.\n", dustdot_names[dd_idx], snap_idx);

                herr_t status = H5Pset_chunk(dd_prop, 2, dd_chunk_dims);
                CHECK_STATUS_AND_RETURN_ON_FAIL(status, (int32_t) status,
                                                "Could not set HDF5 chunking for %s at snapshot %d.\n", dustdot_names[dd_idx], snap_idx);

                hid_t dd_dataset = H5Dcreate2(file_id, full_field_name, H5T_NATIVE_FLOAT, dd_dataspace,
                                              H5P_DEFAULT, dd_prop, H5P_DEFAULT);
                CHECK_STATUS_AND_RETURN_ON_FAIL(dd_dataset, (int32_t) dd_dataset,
                                                "Could not create %s dataset at snapshot %d.\n", dustdot_names[dd_idx], snap_idx);

                CREATE_STRING_ATTRIBUTE(dd_dataset, "Description", dustdot_desc[dd_idx], MAX_STRING_LEN);
                CREATE_STRING_ATTRIBUTE(dd_dataset, "Units", "Msun/yr", MAX_STRING_LEN);

                status = H5Dclose(dd_dataset);
                CHECK_STATUS_AND_RETURN_ON_FAIL(status, (int32_t) status,
                                                "Failed to close %s dataset at snapshot %d.\n", dustdot_names[dd_idx], snap_idx);

                status = H5Pclose(dd_prop);
                CHECK_STATUS_AND_RETURN_ON_FAIL(status, (int32_t) status,
                                                "Failed to close property list for %s at snapshot %d.\n", dustdot_names[dd_idx], snap_idx);

                status = H5Sclose(dd_dataspace);
                CHECK_STATUS_AND_RETURN_ON_FAIL(status, (int32_t) status,
    ...
    for(int32_t snap_idx = 0; snap_idx < run_params->NumSnapOutputs; snap_idx++) {
        ...
        MALLOC_GALAXY_OUTPUT_INNER_ARRAY(snap_idx, ColdDust);
        MALLOC_GALAXY_OUTPUT_INNER_ARRAY(snap_idx, HotDust);
        ...
        // Special allocation for 2D DustDot arrays: size = buffer_size * STEPS
        save_info->buffer_output_gals[snap_idx].DustDotForm = malloc(
            (size_t)save_info->buffer_size * (size_t)STEPS * sizeof(float));
        save_info->buffer_output_gals[snap_idx].DustDotGrowth = malloc(
            (size_t)save_info->buffer_size * (size_t)STEPS * sizeof(float));
        save_info->buffer_output_gals[snap_idx].DustDotDestruct = malloc(
            (size_t)save_info->buffer_size * (size_t)STEPS * sizeof(float));
        if(save_info->buffer_output_gals[snap_idx].DustDotForm == NULL ||
                                   ...
                                   const struct params *run_params)
{
    ...
    for(int32_t snap_idx = 0; snap_idx < run_params->NumSnapOutputs; snap_idx++) {
        ...
        FREE_GALAXY_OUTPUT_INNER_ARRAY(snap_idx, ColdDust);
        FREE_GALAXY_OUTPUT_INNER_ARRAY(snap_idx, HotDust);
                                ...
                                char (*field_units)[MAX_STRING_LEN], hsize_t *field_dtypes)
{
    ...
    char tmp_names[NUM_OUTPUT_FIELDS][MAX_STRING_LEN] = {"SnapNum", "Type", "GalaxyIndex", "CentralGalaxyIndex", "SAGEHaloIndex",
                                                         ...
                                                         "tcool", "tff", "tcool_over_tff", "tdeplete", "RcoolToRvir", "TimeOfInfall", "FFBRegime", "mdot_cool", "mdot_stream", "ColdDust", "HotDust", "CGMDust", "EjectedDust", "TotalDust"};
    ...
    char tmp_descriptions[NUM_OUTPUT_FIELDS][MAX_STRING_LEN] = {"Snapshot the galaxy is located at.",
                                                                ...
                                                                "Mass of cold dust in the galaxy.", "Mass of hot dust in the galaxy.", "Mass of dust in the circum-galactic medium (CGM).", "Mass of ejected dust from the galaxy.", "Total dust mass (cold + hot + CGM + ejected)."};
    ...
    hsize_t tmp_dtype[NUM_OUTPUT_FIELDS] = {H5T_NATIVE_INT, H5T_NATIVE_INT, H5T_NATIVE_LLONG, H5T_NATIVE_LLONG, H5T_NATIVE_INT,
                                            ...
                                            H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_INT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT};
#endif
                                       ...
                                       const struct params *run_params)
{
    ...
    save_info->buffer_output_gals[output_snap_idx].ColdDust[gals_in_buffer] = g->ColdDust;
    save_info->buffer_output_gals[output_snap_idx].HotDust[gals_in_buffer] = g->HotDust;
    save_info->buffer_output_gals[output_snap_idx].CGMDust[gals_in_buffer] = g->CGMDust;
    ...
    // Copy DustDot arrays - convert from code units to Msun/yr (same as SFR conversion)
    // The 2D buffer is laid out as [gal_idx * STEPS + step_idx]
    {
        float rate_conversion = (float)(run_params->UnitMass_in_g / run_params->UnitTime_in_s 
                                        * SEC_PER_YEAR / SOLAR_MASS);
        int64_t dustdot_offset = gals_in_buffer * STEPS;
        for(int32_t step = 0; step < STEPS; step++) {
            save_info->buffer_output_gals[output_snap_idx].DustDotForm[dustdot_offset + step] = g->DustDotForm[step] * rate_conversion;
            save_info->buffer_output_gals[output_snap_idx].DustDotGrowth[dustdot_offset + step] = g->DustDotGrowth[step] * rate_conversion;
            save_info->buffer_output_gals[output_snap_idx].DustDotDestruct[dustdot_offset + step] = g->DustDotDestruct[step] * rate_conversion;
        }
    }
                             ...
                             struct save_info *save_info, const struct params *run_params)
{
    ...
    EXTEND_AND_WRITE_GALAXY_DATASET(ColdDust);
    EXTEND_AND_WRITE_GALAXY_DATASET(HotDust);
    ...
    // Write 2D DustDot datasets (separate from 1D fields)
    {
        const char *dustdot_names[3] = {"DustDotForm", "DustDotGrowth", "DustDotDestruct"};
        float *dustdot_buffers[3] = {
            save_info->buffer_output_gals[snap_idx].DustDotForm,
            save_info->buffer_output_gals[snap_idx].DustDotGrowth,
            save_info->buffer_output_gals[snap_idx].DustDotDestruct
        };

        for(int dd_idx = 0; dd_idx < 3; dd_idx++) {
            char dd_field_name[2*MAX_STRING_LEN];
            snprintf(dd_field_name, 2*MAX_STRING_LEN - 1, "Snap_%d/%s", run_params->ListOutputSnaps[snap_idx], dustdot_names[dd_idx]);

            hid_t dd_dataset_id = H5Dopen2(save_info->file_id, dd_field_name, H5P_DEFAULT);
            if(dd_dataset_id < 0) {
                fprintf(stderr, "Could not access %s dataset for output snapshot %d.\n", dustdot_names[dd_idx], snap_idx);
                return (int32_t) dd_dataset_id;
            }

            // 2D dimensions: [ngal, STEPS]
            hsize_t dd_dims_extend[2] = {(hsize_t)num_to_write, (hsize_t)STEPS};
            hsize_t dd_old_dims[2] = {(hsize_t)num_already_written, (hsize_t)STEPS};
            hsize_t dd_new_dims[2] = {dd_old_dims[0] + dd_dims_extend[0], (hsize_t)STEPS};

            status = H5Dset_extent(dd_dataset_id, dd_new_dims);
            if(status < 0) {
                fprintf(stderr, "Could not resize %s dataset dimensions for snapshot %d.\n", dustdot_names[dd_idx], snap_idx);
                return (int32_t) status;
            }

            hid_t dd_filespace = H5Dget_space(dd_dataset_id);
            if(dd_filespace < 0) {
                fprintf(stderr, "Could not get dataspace for %s at snapshot %d.\n", dustdot_names[dd_idx], snap_idx);
                return (int32_t) dd_filespace;
            }

            // Select hyperslab: start at [old_ngal, 0], count [num_to_write, STEPS]
            hsize_t dd_start[2] = {dd_old_dims[0], 0};
            status = H5Sselect_hyperslab(dd_filespace, H5S_SELECT_SET, dd_start, NULL, dd_dims_extend, NULL);
            if(status < 0) {
                fprintf(stderr, "Could not select hyperslab for %s at snapshot %d.\n", dustdot_names[dd_idx], snap_idx);
                return (int32_t) status;
            }

            hid_t dd_memspace = H5Screate_simple(2, dd_dims_extend, NULL);
            if(dd_memspace < 0) {
                fprintf(stderr, "Could not create memory space for %s at snapshot %d.\n", dustdot_names[dd_idx], snap_idx);
                return (int32_t) dd_memspace;
            }

            status = H5Dwrite(dd_dataset_id, H5T_NATIVE_FLOAT, dd_memspace, dd_filespace, H5P_DEFAULT,
                              dustdot_buffers[dd_idx]);
            CHECK_STATUS_AND_RETURN_ON_FAIL(status, (int32_t) status,
