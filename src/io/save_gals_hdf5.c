...
/* HDF5 is self-describing; each dataset gets attributes for units and description.
   The group_id is validated by the caller before these macros are invoked. */
...
/* Unlike binary output (array-of-structs), the HDF5 workflow uses struct-of-arrays.
   This macro allocates space for each inner field array. */
...
/* Creates the HDF5 file with layout File -> Snap_N -> field datasets.
   All handles are stored in save_info for use during writing. */
int32_t initialize_hdf5_galaxy_files(const int filenr, struct save_info *save_info, const struct params *run_params)
{
    char buffer[3*MAX_STRING_LEN]; /* 3× because OutputDir + FileNameGalaxies can each be MAX_STRING_LEN */
    ...
    for(int32_t snap_idx = 0; snap_idx < run_params->NumSnapOutputs; snap_idx++) {
        ...
        for(int32_t field_idx = 0; field_idx < NUM_OUTPUT_FIELDS; field_idx++) {
            ...
            /* start with 0-length dimension; extended before each write (requires chunking) */
        ...
        if(run_params->SaveFullSFH) {
            herr_t sfh_status; /* distinct name to avoid shadowing macro-local 'status' */
    ...
    save_info->num_gals_in_buffer = mycalloc(run_params->NumSnapOutputs, sizeof(save_info->num_gals_in_buffer[0]));
    ...
    /* malloc all inner field arrays for the struct-of-arrays HDF5 output */
    for(int32_t snap_idx = 0; snap_idx < run_params->NumSnapOutputs; snap_idx++) {
        ...
        if(run_params->SaveFullSFH) {
            ...
            if(save_info->buffer_output_gals[snap_idx].SFHMassDisk == NULL ||
               save_info->buffer_output_gals[snap_idx].SFHMassBulge == NULL) {
                ...
                /* free whichever allocation succeeded before returning */
                                   ...
                                   const struct params *run_params)
{
    ...
    /* header written here (not in initialize) because it needs the per-tree galaxy counts */
...
int32_t create_hdf5_master_file(const struct params *run_params)
{
    ...
    /* track galaxy counts across all files for all snapshots (not just output ones) */
    int64_t *ngals_allfiles_snap = mycalloc(run_params->SimMaxSnaps, sizeof(*(ngals_allfiles_snap)));
    ...
    /* master file layout: f["Core0"]["Snap_63"]["StellarMass"] — link to per-task root group */
    ...
    /* NULL forest_info: master file has no per-tree knowledge at write time */
...
/* Field names match the binary reader (e.g., tests/sagediff.py) to allow cross-format comparison. */
                                       ...
                                       const struct params *run_params)
{
    ...
    /* infall properties (zero for centrals) */
    if(g->Type != 0) {
...
/* NULL pointer dereference in SIZEOF_STRUCT_FIELD is intentional: sizeof is compile-time only and
   never actually dereferences the pointer, so no invalid memory access occurs. */
#define SIZEOF_STRUCT_FIELD(field)    (sizeof(((struct HDF5_GALAXY_OUTPUT *) NULL)->field[0]))

/* Datasets were created with unlimited dimensions; before each write they must be extended via
   H5Dset_extent, then written via a hyperslab selection — required by the HDF5 chunked storage model. */
...
/* num_to_write is explicit because this is called both at buffer-full and at finalization (remaining galaxies). */
int32_t trigger_buffer_write(const int32_t snap_idx, const int32_t num_to_write, const int64_t num_already_written,
                             struct save_info *save_info, const struct params *run_params)
{
    herr_t status;

    /* HDF5 requires array-form arguments for dimension values, not inline scalars */
    ...
    /* stack of macros because C can't iterate over struct members */
#ifdef USE_SAGE_IN_MCMC_MODE
