...
/* snprintf wrapper that returns -1 and prints an error instead of silently truncating. */
...
/* Returns a malloc'd string describing the elapsed time between t0 and t1 in
   the largest applicable unit (days/hrs/mins/secs). Caller must free(). */
char *get_time_string(struct timeval t0, struct timeval t1)
{
    ...
    if(time_string == NULL) {
        fprintf(stderr, "Error: Could not allocate memory for time string in '%s'\n", __FUNCTION__);
        return NULL;
    }
    double timediff = t1.tv_sec - t0.tv_sec;
    double ratios[] = {24 * 3600.0, 3600.0, 60.0, 1};
    const char *units[] = {"days", "hrs", "mins", "secs"};
...
{
    /* Check absolute difference first to handle near-zero comparisons correctly. */
