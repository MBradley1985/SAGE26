...
void test_br06_reference_values() {
    ...
    /* Re-pinned when P_0 moved from 4.54e4 to 4.3e4 K cm^-3 to match the
     * Blitz & Rosolowsky (2006) eq. 13 non-interacting fit. */

    ASSERT_CLOSE(2.295262814e-01, calculate_molecular_fraction_BR06(10.0f, 100.0f, 3000.0f),
                 GOLDEN_RTOL, "f_H2(10, 100, 3000 pc)");
    ASSERT_CLOSE(1.421214268e-02, calculate_molecular_fraction_BR06(1.0f, 10.0f, 2000.0f),
                 GOLDEN_RTOL, "f_H2(1, 10, 2000 pc)");
    ASSERT_CLOSE(8.653799891e-01, calculate_molecular_fraction_BR06(100.0f, 1000.0f, 4000.0f),
                 GOLDEN_RTOL, "f_H2(100, 1000, 4000 pc)");
    ASSERT_CLOSE(4.085152759e-04, calculate_molecular_fraction_BR06(0.3f, 0.05f, 5000.0f),
...
void test_radial_integration_reference() {
    ...
    float h2 = calculate_molecular_fraction_radial_integration(0, gal, &rp, NULL);
    ASSERT_CLOSE(2.974629169e-03, h2, GOLDEN_RTOL,
...
static void run_sf_driver(struct GALAXY *gal, const struct params *rp,
    ...
    gal[0].Mvir = 100.0;
    gal[0].SnapNum = 0;
    starformation_and_feedback(0, 0, /*time*/ 1.0, /*dt*/ 1e-6, /*halonr*/ 0, /*halo_snapnum*/ 0, /*step*/ 0,
                               gal, rp);
}
