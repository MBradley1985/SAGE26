...
| `FeedbackFreeModeOn` | int | no | `1` | Feedback-free burst galaxies: 0=off; 1=Li+24 sigmoid; 2=BK25 (Ishiyama+21 c); 3=BK25 (ConcentrationOn method); 4=BK25 + log-normal c scatter; 5=Li+24 sharp; 6=Li+24 sigmoid + H₂ SF; 7=BK25 log-normal c scatter + H₂ SF; 8=Dekel+23 free-fall-time criterion, eq. (4) applied directly: FFB when `t_ff < FFBFeedbackDelayMyr` (eq. 3), where `t_ff = 0.84 Myr (n/10^3.5 cm^-3)^-1/2` (eq. 4) and `n` is the galaxy's cold-gas disc density (`ColdGas` within a `DiskScaleRadius` sphere, μ=1.2), scaled up by `FFBCloudClumping` to the density of the actual star-forming clumps. |
...
| `FFBCloudClumping` | double | no | `1.0` | Density enhancement `c` (>= 1) of the star-forming clumps above the galaxy's disc-averaged cold-gas density. Used only by `FeedbackFreeModeOn=8`, which multiplies the disc density `n` by `c` before evaluating eq. (4). On `millennium_noffb` at `z~8.5` the median disc density is `n ~ 50 cm^-3` against `n_fbk ~ 2.2e3 cm^-3` (eq. 5), so `c ~ O(10)` is enough to bring typical halos across the threshold -- unlike a clumping factor applied to the halo's *mean virial* density, which would need `c ~ O(10^4)`. |
