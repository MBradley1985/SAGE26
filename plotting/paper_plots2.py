...
def plot_gas_mass_functions(sim_configs, snapshot, output_dir):
    ...
    for i, sim_config in enumerate(sim_configs):
        ...
        try:
            ...
            H1Gas = read_hdf_ultra_optimized(snap_num=snapshot, param='H1gas', directory=directory) * 1.0e10 / hubble_h  # Read directly from model
...
def plot_h2_fraction_vs_stellar_mass(sim_configs, snapshot, output_dir):
    ...
    for i, sim_config in enumerate(sim_configs):
        ...
        try:
            ...
            H2Gas = read_hdf_ultra_optimized(snap_num=snapshot, param='H2gas', directory=directory) * 1.0e10 / hubble_h
            ColdGas = read_hdf_ultra_optimized(snap_num=snapshot, param='ColdGas', directory=directory) * 1.0e10 / hubble_h
            HI_Gas = read_hdf_ultra_optimized(snap_num=snapshot, param='H1gas', directory=directory) * 1.0e10 / hubble_h  # Read directly from model
            SfrDisk = read_hdf_ultra_optimized(snap_num=snapshot, param='SfrDisk', directory=directory)
            SfrBulge = read_hdf_ultra_optimized(snap_num=snapshot, param='SfrBulge', directory=directory)
            Type = read_hdf_ultra_optimized(snap_num=snapshot, param='Type', directory=directory)
            ...
            hi_gas_sel = HI_Gas[w]  # Use HI read directly from model
    ...
    if len(sim_configs) > 0:
        ...
        try:
            ...
            if len(w_z1) > 0:
                ...
                H1Gas_z1 = read_hdf_ultra_optimized(snap_num=z1_snapshot, param='H1gas', directory=directory) * 1.0e10 / hubble_h
                hi_gas_z1 = H1Gas_z1[w_z1]  # Use HI read directly from model
...
def plot_h2_fraction_vs_halo_mass(sim_configs, snapshot, output_dir):
    ...
    for i, sim_config in enumerate(sim_configs):
        ...
        try:
            ...
            HI_Gas = read_hdf_ultra_optimized(snap_num=snapshot, param='H1gas', directory=directory) * 1.0e10 / hubble_h  # Read directly from model
            HI_Gas_broken = HI_Gas  # Same as HI_Gas now
