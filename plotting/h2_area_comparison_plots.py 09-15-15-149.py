def plot_property_evolution_grid(z_min=0.0, z_max=5.0, n_z=10):
    """
    Create a grid of plots showing the evolution of key properties as a function of redshift (z=0-2)
    Properties: Stellar Mass, H2, HI, Cold Gas, Black Holes, Quiescent fraction, Metallicity, SFR, sSFR, Radius
    """
    properties = [
        ("Stellar Mass", "StellarMass", r"$M_\star$", lambda sm, **_: np.median(sm)),
        ("H2 Mass", "H2gas", r"$M_{\mathrm{H}_2}$", lambda h2, **_: np.median(h2)),
        ("HI Mass", "H1gas", r"$M_{\mathrm{HI}}$", lambda hi, **_: np.median(hi)),
        ("Cold Gas Mass", "ColdGas", r"$M_{\mathrm{cold}}$", lambda cg, **_: np.median(cg)),
        ("Black Hole Mass", "BlackHoleMass", r"$M_{\mathrm{BH}}$", lambda bh, **_: np.median(bh)),
        ("Quiescent Fraction", ("StellarMass", "SfrDisk", "SfrBulge"), r"Quiescent Fraction", None),
        ("Metallicity", ("ColdGas", "MetalsColdGas"), r"$12+\log_{10}(O/H)$", None),
        # ("SFR", ("SfrDisk", "SfrBulge"), r"SFR", lambda sd, sb, **_: np.median(sd+sb)),
        ("sSFR", ("StellarMass", "SfrDisk", "SfrBulge"), r"sSFR", None),
        # ("Radius", "DiskRadius", r"Radius (kpc)", lambda r, **_: np.median(r)),
    ]
    z_vals = np.linspace(z_min, z_max, n_z)
    fig, axes = plt.subplots(2, 4, figsize=(22, 9), sharex=True)
    axes = axes.flatten()
    mass_panel_indices = [0, 1, 2, 3, 4, 7]  # Indices for mass-related properties
    legend_handles = []
    legend_labels = []
    for pidx, (prop_name, param, ylabel, reducer) in enumerate(properties):
        ax = axes[pidx]
            ...
            for area_option in AREA_OPTIONS:
                ...
                for z in z_vals:
                    try:
                        ...
                        z_diff = [abs(zz - z) for zz in DEFAULT_REDSHIFTS]
                        ...
                        if isinstance(param, str):
                            arr = read_hdf(directory, snap_num=snap_str, param=param)
                            if arr is None:
                                yvals.append(np.nan)
                                continue
                            if param == "DiskRadius":
                                arr = arr * 1.0e3  # Convert to pc
                            else:
                                arr = arr * 1.0e10 / MILLENNIUM_HUBBLE_H
                            mask = arr > 0
                            if np.sum(mask) == 0:
                                yvals.append(np.nan)
                                continue
                            yvals.append(reducer(arr[mask]))
                        elif prop_name == "HI Mass":
                            hi = read_hdf(directory, snap_num=snap_str, param="H1gas")
                            hi = hi * 1.0e10 / MILLENNIUM_HUBBLE_H
                            mask = hi > 0
                            if np.sum(mask) == 0:
                                yvals.append(np.nan)
                                continue
                            yvals.append(np.median(hi[mask]))
                        elif prop_name == "Quiescent Fraction":
                            sm = read_hdf(directory, snap_num=snap_str, param="StellarMass")
                            sd = read_hdf(directory, snap_num=snap_str, param="SfrDisk")
                            sb = read_hdf(directory, snap_num=snap_str, param="SfrBulge")
                            if sm is None or sd is None or sb is None:
                                yvals.append(np.nan)
                                continue
                            sm = sm * 1.0e10 / MILLENNIUM_HUBBLE_H
                            sfr = sd + sb
                            mask = sm > 0
                            sm = sm[mask]
                            sfr = sfr[mask]
                            ssfr = sfr / sm
                            quiescent = np.sum(ssfr < 1e-11) / len(ssfr) if len(ssfr) > 0 else np.nan
                            yvals.append(quiescent)
                        elif prop_name == "Metallicity":
                            cg = read_hdf(directory, snap_num=snap_str, param="ColdGas")
                            met = read_hdf(directory, snap_num=snap_str, param="MetalsColdGas")
                            if cg is None or met is None:
                                yvals.append(np.nan)
                                continue
                            cg = cg * 1.0e10 / MILLENNIUM_HUBBLE_H
                            met = met * 1.0e10 / MILLENNIUM_HUBBLE_H
                            mask = cg > 1e-6
                            metallicity = met[mask] / cg[mask]
                            Z_sun = 0.0134
                            metallicity_12OH = 8.69 + np.log10(metallicity / Z_sun)
                            valid = np.isfinite(metallicity_12OH) & (metallicity_12OH > 7.0) & (metallicity_12OH < 10.0)
                            yvals.append(np.median(metallicity_12OH[valid]) if np.sum(valid) > 0 else np.nan)
                        elif prop_name == "sSFR":
                            sm = read_hdf(directory, snap_num=snap_str, param="StellarMass")
                            sd = read_hdf(directory, snap_num=snap_str, param="SfrDisk")
                            sb = read_hdf(directory, snap_num=snap_str, param="SfrBulge")
                            ...
                            sfr = sfr[mask]
                            ssfr = sfr / sm
                            yvals.append(np.median(ssfr[ssfr > 0]) if np.sum(ssfr > 0) > 0 else np.nan)
                        else:
                            yvals.append(np.nan)
                ...
                if len(yvals_arr) == len(z_vals) and np.any(~np.isnan(yvals_arr)):
                    line, = ax.plot(z_vals, yvals_arr, color=color, linestyle=AREA_OPTION_LINESTYLES[area_option],
                        linewidth=2, label=label, alpha=0.8)
                    if pidx == 0:
                        legend_handles.append(line)
                        legend_labels.append(label)
        ax.set_ylabel(ylabel, fontsize=12)
        if pidx in mass_panel_indices:
            ax.set_yscale('log')
        if pidx >= 5:
            ax.set_xlabel('Redshift', fontsize=12)
        # No per-panel legend
        ax.invert_xaxis()
    # Place a single legend at the bottom, outside the grid
    fig.tight_layout(rect=[0, 0.05, 1, 1])
    fig.legend(legend_handles, legend_labels, loc='lower center', bbox_to_anchor=(0.5, 0.0),
               ncol=8, fontsize=11, framealpha=0.9)
    os.makedirs(OutputDir, exist_ok=True)
    output_path = os.path.join(OutputDir, 'property_evolution_grid_z0-5.pdf')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\n  Saved: {output_path}")
    plt.close()
def plot_smf(target_z=0.0):
    """
    Plot the stellar mass function (SMF) for different H2 prescriptions and area options
    """
    ...
    snap_str = f'Snap_{best_snap}'
    print(f"\n{'='*60}")
    print(f"Creating stellar mass function (SMF) plot for z ~ {target_z}")
    ...
    for prescription in H2_PRESCRIPTIONS:
        ...
        for area_option in AREA_OPTIONS:
            ...
            try:
                ...
                mask = (stellar_mass > 0) & (galaxy_type == 0)
                log_stellar_mass = np.log10(stellar_mass[mask])
                # Histogram for SMF
                hist, _ = np.histogram(log_stellar_mass, bins=mass_bins)
                # Convert to number density: divide by bin width and volume (Mpc^3)
                volume = MILLENNIUM_BOXSIZE**3  # (Mpc/h)^3
                smf = hist / (volume * (mass_bins[1] - mass_bins[0]))
                if np.any(smf > 0):
                    label = f'{prescription} {AREA_OPTION_LABELS[area_option]}'
                    ax.plot(bin_centers, np.log10(smf + 1e-10), color=color, linestyle=linestyle,
                            linewidth=2, label=label, alpha=0.8)
            except Exception as e:
                print(f"    Error: {e}")
                continue
    ...
    os.makedirs(OutputDir, exist_ok=True)
    output_path = os.path.join(OutputDir, f'smf_z{target_z:.1f}.pdf')
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\n  Saved: {output_path}")
    plt.close()
def plot_bh_mass_vs_stellar_mass(target_z=0.0):
    """
    Plot black hole mass vs. stellar mass for different H2 prescriptions and area options
    """
    mass_bins = np.arange(8.0, 12.5, 0.2)
    z_diff = [abs(z - target_z) for z in DEFAULT_REDSHIFTS]
    best_snap = z_diff.index(min(z_diff))
    ...
    snap_str = f'Snap_{best_snap}'
    print(f"\n{'='*60}")
    print(f"Creating black hole mass vs. stellar mass plot for z ~ {target_z}")
    print(f"Using {snap_str} (z={actual_z:.2f})")
    print(f"{'='*60}")
    fig, ax = plt.subplots(figsize=(10, 7))
    for prescription in H2_PRESCRIPTIONS:
        ...
        for area_option in AREA_OPTIONS:
            ...
            try:
                stellar_mass = read_hdf(directory, snap_num=snap_str, param='StellarMass')
                bh_mass = read_hdf(directory, snap_num=snap_str, param='BlackHoleMass')
                galaxy_type = read_hdf(directory, snap_num=snap_str, param='Type')
                if stellar_mass is None or bh_mass is None or galaxy_type is None:
                    continue
                stellar_mass = stellar_mass * 1.0e10 / MILLENNIUM_HUBBLE_H
                bh_mass = bh_mass * 1.0e10 / MILLENNIUM_HUBBLE_H
                mask = (stellar_mass > 0) & (bh_mass > 0) & (galaxy_type == 0)
                stellar_mass_filtered = stellar_mass[mask]
                bh_mass_filtered = bh_mass[mask]
                if len(stellar_mass_filtered) == 0:
                    continue
                log_stellar_mass = np.log10(stellar_mass_filtered)
                log_bh_mass = np.log10(bh_mass_filtered)
                # Bin by stellar mass
                bin_centers = 0.5 * (mass_bins[:-1] + mass_bins[1:])
                bin_medians = []
                valid_bins = []
                for i in range(len(mass_bins)-1):
                    ...
                    if np.sum(bin_mask) >= 5:
                        bin_medians.append(np.median(log_bh_mass[bin_mask]))
                ...
                continue
    ax.set_xlabel(r'$\log_{10}(M_\star / \mathrm{M}_\odot)$', fontsize=14)
    ax.set_ylabel(r'$\log_{10}(M_\mathrm{BH} / \mathrm{M}_\odot)$', fontsize=14)
    ax.set_xlim(8.0, 12.0)
    ax.set_ylim(bottom=5)
    ax.legend(loc='best', fontsize=9, framealpha=0.9, ncol=2)
    os.makedirs(OutputDir, exist_ok=True)
    output_path = os.path.join(OutputDir, f'bh_mass_vs_stellar_mass_z{target_z:.1f}.pdf')
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\n  Saved: {output_path}")
    plt.close()
...
def plot_sfr_surface_density_vs_cold_gas_surface_density(target_z=0.0):
    ...
    for prescription in H2_PRESCRIPTIONS:
        ...
        for area_option in AREA_OPTIONS:
            ...
            try:
                ...
                disk_radius_kpc = disk_radius * 1.0e3  # in kpc
                disk_area_pc2 = np.pi * disk_radius_pc**2
                disk_area_kpc2 = np.pi * disk_radius_kpc**2
                sigma_cold_gas = cold_gas / disk_area_pc2  # M_sun/pc^2
                sigma_sfr = sfr / disk_area_kpc2  # M_sun/yr/pc^2
    ...
    ax.set_ylabel(r'$\Sigma_{\mathrm{SFR}}$ [M$_\odot$/yr/kpc$^2$]', fontsize=14)
...
OutputDir = './output/testmodels/Millennium_FFB/plots/'
ObsDataDir = '../SHARKV2/data/Gas/'
    ...
    for prescription in H2_PRESCRIPTIONS:
        ...
        for area_option in AREA_OPTIONS:
            ...
            try:
                ...
                cold_gas = read_hdf(directory, snap_num=snap_str, param='ColdGas')
                galaxy_type = read_hdf(directory, snap_num=snap_str, param='Type')
                h1_gas = read_hdf(directory, snap_num=snap_str, param='H1gas')
                
                if stellar_mass is None or h2_gas is None or cold_gas is None or h1_gas is None:
                    continue
                ...
                cold_gas = cold_gas * 1.0e10 / MILLENNIUM_HUBBLE_H
                h1_gas = h1_gas * 1.0e10 / MILLENNIUM_HUBBLE_H
...
if __name__ == '__main__':
    ...
    for prescription in H2_PRESCRIPTIONS:
        for area_option in AREA_OPTIONS:
            directory = f'./output/testmodels/Millennium_{prescription.lower()}_{area_option}/'
    ...
    print("\n" + "="*60)
    print("Creating black hole mass vs. stellar mass plot...")
    print("="*60)
    plot_bh_mass_vs_stellar_mass(target_z=0.0)
    print("\n" + "="*60)
    print("Creating stellar mass function (SMF) plot...")
    print("="*60)
    plot_smf(target_z=0.0)
    print("\n" + "="*60)
    print("Creating property evolution grid (z=0-10)...")
    print("="*60)
    plot_property_evolution_grid(z_min=0.0, z_max=10.0, n_z=10)
