...
def trace_ffb_fates():
    ...
    for gal_idx, tracker in galaxy_tracker.items():
        ...
        # Check if survived to final snapshot (z~0.02 for Mini-Uchuu)
        # These galaxies are still alive when the simulation ends
...
def print_fate_statistics(galaxy_tracker, fates):
    ...
    fate_names = {
        'survived_z0': f'Survived to z~{redshifts[LastSnap]:.2f} (end of simulation)',
...
def plot_fate_diagnostics(galaxy_tracker, fates, z_array, ffb_present):
    ...
    fate_labels = ['Survived to end', 'Minor merger', 'Major merger', 
