...
MODEL_CONFIGS = [
    ...
    },
    {
        ...
        'linewidth': 3.5,              # Same thickness as SAGE26
        'alpha': 1.0,                # Transparency
        'boxsize': MILLENNIUM_BOXSIZE,             # Box size in h^-1 Mpc for this model
        'volume_fraction': 1.0,      # Fraction of the full volume output by the model
    ...
    # NEW: Add your comparison model here
    {
        ...
        'dir': '../../SAGE_BROKEN/sage-model/output/millennium/',  # UPDATE: Path to your comparison model directory
        'color': 'red',            # Color for plotting
    ...
    { 
        'name': 'miniUchuu', 
        'dir': './output/testmodels/miniUchuu_FFB/',
        'color': 'purple', 
        'linestyle': '--',
        'boxsize': 400,
    ...
    try:
        ...
        if file_type == 'farmer':
            ...
            obs_data_by_z[obs['z']] = {
                ...
                'label': obs['label'],
                'color': obs['color'],
                'linestyle': obs.get('linestyle', '-'),
                'linewidth': obs.get('linewidth', 2),
                'type': file_type
            }
...
def add_observational_data_with_baldry(ax, z_low, z_high, obs_data, muzzin_data, santini_data, wright_data, harvey_data, obs_datasets_in_legend, sim_datasets_in_legend, show_shark=True):
    ...
    if is_lowest_redshift_bin(z_low, z_high):
        try:
            ...
            yerr_lower = log_phi_center - log_phi_lower
            yerr_upper = log_phi_upper - log_phi_center
            ...
            # Plot as data points with error bars
            baldry_plot = ax.errorbar(Baldry_xval, log_phi_center,
                           yerr=[yerr_lower, yerr_upper],
                           fmt='o', color='gray', markersize=5,
                           label=baldry_label, alpha=0.7, capsize=0, zorder=2)

            obs_added_info['baldry'] = True
            if baldry_label is not None:
                obs_datasets_in_legend['baldry'] = True
                obs_legend_items.append((baldry_plot, baldry_label))
        ...
        try:
            ...
            if np.any(mask):
                ...
                muzzin_plot = ax.plot(muzzin_masses_plot, muzzin_logphi_plot,
                       ...
                       label=muzzin_label, alpha=0.7, zorder=2)
        ...
        try:
            ...
            if np.any(mask):
                ...
                santini_plot = ax.errorbar(santini_masses_plot, santini_logphi_plot,
                           ...
                           label=santini_label, alpha=0.7, capsize=0, zorder=2)
        ...
        try:
            ...
            if np.any(mask):
                ...
                harvey_plot = ax.errorbar(h_mass, h_log_phi,
                           ...
                           label=harvey_label, alpha=0.7, capsize=0, zorder=2)
            ...
            if 'y_lower' in obs_info and 'y_upper' in obs_info:
                ...
                if np.any(mask):
                    ...
                    smfvals_plot = ax.errorbar(obs_masses_plot, obs_phi_log,
                               ...
                               label=smfvals_label, alpha=0.7, capsize=0, linewidth=0,
                ...
                if np.any(mask):
                    ...
                    smfvals_plot = ax.plot(obs_masses_plot, obs_phi_log,
                           ...
                           label=smfvals_label, alpha=0.7, markersize=marker_size, zorder=2)
            ...
            if 'y_lower' in obs_info and 'y_upper' in obs_info:
                ...
                if np.any(mask):
                    ...
                    farmer_plot = ax.errorbar(obs_masses_plot, obs_phi_log,
                               ...
                               xerr=None,  # NO horizontal error bars
                               fmt=marker_style, color=obs_info['color'], markersize=marker_size,
                               label=farmer_label, alpha=0.7, capsize=0, linewidth=0,
                               elinewidth=1.5, zorder=2)  # Make error bar lines a bit thicker
                ...
                if np.any(mask):
                    ...
                    farmer_plot = ax.plot(obs_masses_plot, obs_phi_log,
                           marker_style, color=obs_info['color'],
                           label=farmer_label, alpha=0.7, markersize=marker_size, zorder=2)
    ...
    if obs_bin is not None and obs_bin in obs_data:
        ...
        if np.any(mask):
            ...
            cosmos_plot = ax.errorbar(obs_masses_plot, log_phi,
                       ...
                       label=cosmos_label, alpha=0.7, capsize=0, zorder=2)
        ...
        try:
            ...
            if np.any(mask):
                ...
                wright_plot = ax.errorbar(wright_masses_plot, wright_logphi_plot,
                           ...
                           label=wright_label, alpha=0.7, capsize=0, zorder=2)
        ...
        if stef_mass is not None:
            ...
            if np.any(valid):
                ...
                stef_plot = ax.errorbar(stef_mass[valid], stef_phi[valid],
                           ...
                           fmt='o', color='gray', markersize=6, alpha=0.7,
                           label=stef_label, capsize=0, linewidth=1.5, zorder=2)
        ...
        if nc_mass is not None:
            ...
            if np.any(valid):
                ...
                nc_plot = ax.errorbar(nc_mass[valid], nc_phi[valid],
                           yerr=[yerr_low, yerr_high],
                           fmt='^', color='gray', markersize=6, alpha=0.7,
                           label=nc_label, capsize=0, linewidth=1.5, zorder=2)
        ...
        if weibel_mass is not None:
            ...
            if np.any(valid):
                ...
                weibel_plot = ax.errorbar(weibel_mass[valid], weibel_phi[valid],
                           yerr=[yerr_low, yerr_high],
                           fmt='v', color='gray', markersize=6, alpha=0.7,
                           label=weibel_label, capsize=0, linewidth=1.5, zorder=2)
        ...
        if kiku_mass is not None:
            ...
            if np.any(valid):
                ...
                kiku_plot = ax.plot(kiku_mass[valid], kiku_phi[valid],
                       marker='<', color='gray', linestyle='', markersize=6, alpha=0.7,
...
def plot_smf_redshift_grid(galaxy_types='all', mass_range=(7, 12),
    ...
    model_configs : list or None
        List of model configurations to plot. If None, uses global MODEL_CONFIGS.
    """
    # Use global MODEL_CONFIGS if specific configs not provided
    ...
    for i, (z_low, z_high, z_center, snapshots) in enumerate(redshift_bins):
        ...
        # Formatting
        ax.text(0.95, 0.95, f'{z_low:.1f} < z < {z_high:.1f}', transform=ax.transAxes,
    ...
    for i in range(min(n_bins, len(axes_flat))):
        ...
        # Only bottom row gets x-tick labels and x-axis title
        if row == n_rows - 1 or i >= n_bins - n_cols:
            ...
            axes_flat[i].tick_params(axis='x', labelbottom=False)

        # Only left column gets y-tick labels and y-labels
        if col == 0:
            axes_flat[i].set_ylabel(r'$\log_{10}\phi\ (\mathrm{Mpc}^{-3}\,\mathrm{dex}^{-1})$', fontsize=16)
            axes_flat[i].tick_params(axis='y', labelleft=True)
        else:
            axes_flat[i].tick_params(axis='y', labelleft=False)
    ...
    if has_ffb_models:
        plt.subplots_adjust(hspace=0, wspace=0, right=0.90)
...
def plot_smf_all_redshift_bins_with_residuals(galaxy_types='all', mass_range=(7, 12), 
                                             ...
                                             figure_title="", model_configs=None):
    """
    ...
    # Main plots get 4 units of height, residual plots get 1 unit
    height_ratios = []
    for i in range(n_rows_main):
        height_ratios.extend([4, 1])  # 4:1 ratio for main:residual

    gs = gridspec.GridSpec(n_rows_main * 2, n_cols, height_ratios=height_ratios,
                          hspace=0, wspace=0, figure=fig)
    ...
    for i, (z_low, z_high, z_center, snapshots) in enumerate(redshift_bins):
        ...
        if sage_2_data is not None and comparison_data is not None:
            ...
            if len(residuals) > 0:
                ...
                residual_ax.axhline(y=0, color='black', linestyle='-', alpha=0.7, linewidth=1)
        ...
        main_ax.text(0.95, 0.95, f'{z_low:.1f} < z < {z_high:.1f}', transform=main_ax.transAxes,
                     fontsize=12, ha='right', va='top')
        main_ax.set_xlim(8, 12.3)
    ...
    for i in range(min(n_bins, len(main_axes))):
        ...
        if row == n_rows_main - 1 or i >= n_bins - n_cols:
            ...
            residual_axes[i].tick_params(axis='x', labelbottom=True)
        else:
            residual_axes[i].tick_params(axis='x', labelbottom=False)

        # Left column gets y-labels, others hide y-tick labels
        if col == 0:
            main_axes[i].set_ylabel(r'$\log_{10}\phi\ (\mathrm{Mpc}^{-3}\,\mathrm{dex}^{-1})$', fontsize=14)
            main_axes[i].tick_params(axis='y', labelleft=True)
            residual_axes[i].tick_params(axis='y', labelleft=True)
        else:
            main_axes[i].tick_params(axis='y', labelleft=False)
            residual_axes[i].tick_params(axis='y', labelleft=False)
...
def plot_smf_all_redshift_bins(galaxy_types='all', mass_range=(7, 12),
                              save_path=None, show_observations=True, figure_title="", model_configs=None, show_gaea=True, show_shark=True, grid_cols=5):
    """
    Create a comprehensive grid plot of stellar mass functions for ALL redshift bins:
    ...
    model_configs : list or None
        List of model configurations to plot. If None, uses global MODEL_CONFIGS.
    grid_cols : int
        Number of columns in the grid layout (default 5). Rows computed automatically.
    """
    # Use global MODEL_CONFIGS if specific configs not provided
    if model_configs is None:
        model_configs = MODEL_CONFIGS
    ...
    # Create grid (15 panels total)
    n_cols = grid_cols
    n_rows = int(np.ceil(15 / n_cols))
...
def create_requested_plots():
    ...
    try:
        ...
        # 3-column versions
        print("\n4. Creating 3-column comprehensive plots...")
        plot_smf_all_redshift_bins(
            galaxy_types='all',
            mass_range=(8, 12),
            save_path=OutputDir + 'comprehensive_sage_c16_only_3col.pdf',
            show_observations=False,
            show_gaea=False,
            show_shark=False,
            grid_cols=3
        )
        print("   ✓ Saved: comprehensive_sage_c16_only_3col.pdf")

        plot_smf_all_redshift_bins(
            galaxy_types='all',
            mass_range=(8, 12),
            save_path=OutputDir + 'comprehensive_sage_c16_with_obs_3col.pdf',
            show_observations=True,
            show_gaea=False,
            show_shark=False,
            grid_cols=3
        )
        print("   ✓ Saved: comprehensive_sage_c16_with_obs_3col.pdf")

        plot_smf_all_redshift_bins(
            galaxy_types='all',
            mass_range=(8, 12),
            save_path=OutputDir + 'comprehensive_sage_c16_shark_gaea_obs_3col.pdf',
            show_observations=True,
            show_gaea=True,
            show_shark=True,
            grid_cols=3
        )
        print("   ✓ Saved: comprehensive_sage_c16_shark_gaea_obs_3col.pdf")
...
if __name__ == "__main__":
    ...
    all_z_model_names = [
        'SAGE C16', 'miniUchuu', 'SAGE26'
    ]
    
    # Helper to filter models
    def get_models_by_names(names):
        models = [m for m in MODEL_CONFIGS if m['name'] in names]
    ...
    for model_config in MODEL_CONFIGS:
        ...
        if not os.path.exists(directory):
            ...
            # We continue anyway to allow the script structure to be correct even if models are missing
            # continue 
    ...
    # We don't filter MODEL_CONFIGS strictly here to allow the user to add paths later
    ...
    try:
        ...
        all_z_models = get_models_by_names(all_z_model_names)
        if not all_z_models:
            print(f"Warning: No models found for All-Z grid. Expected: {all_z_model_names}")

        plot_smf_all_redshift_bins(galaxy_types='all',
                                save_path=OutputDir + 'comprehensive_all.pdf',
                                model_configs=all_z_models)
        plot_smf_all_redshift_bins(galaxy_types='all',
                                save_path=OutputDir + 'comprehensive_all_3col.pdf',
                                model_configs=all_z_models, grid_cols=3)
        ...
        print("1. sage_smf_redshift_grid_low_z_all.pdf - All galaxies (z=0-4) - 3x3 grid")
        print("2. sage_smf_redshift_grid_high_z_all.pdf - All galaxies (z=4-10) - 2x3 grid")
        print("3. comprehensive_all.pdf - All galaxies (z=0-12) - 3x5 grid")
        ...
        print("\nHIGH-Z FIGURE (z=5.5-13): 2×3 grid")
        print("  Bins: z=5.5-6.5, z=6.5-7.5, z=7.5-8.5")
        print("        z=8.5-9.5, z=9.5-10.5, z=12.0-13.0")
