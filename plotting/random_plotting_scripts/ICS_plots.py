...
if __name__ == '__main__':
    ...
    for idx, snap in enumerate(ICS_snaps):
        ...
        x_bins = np.linspace(10, 15, 50)
        y_bins = np.linspace(-1, 1.0, 50)
        ...
        # A sigma of 1.0 or 1.5 usually smooths the jaggedness without destroying real physical structure.
        smoothing_sigma = 1.2 
        H_smoothed = ndimage.gaussian_filter(H, sigma=smoothing_sigma)
        ...
        # Plot filled contours using the SMOOTHED histogram
        if H_smoothed.max() > 1:
            levels = np.logspace(0, np.log10(H_smoothed.max()), 10)
            norm = mcolors.LogNorm(vmin=1, vmax=H_smoothed.max())
            
            contour = ax.contourf(X, Y, H_smoothed, levels=levels, cmap='plasma', norm=norm, extend='max')

        # Calculate median in bins
        bin_width = 0.2
        bin_edges = np.arange(10, 15, bin_width)
        ...
        for i in range(len(bin_edges) - 1):
            ...
            if np.sum(mask) > 2:  # Only compute if enough galaxies
    ...
    for idx, snap in enumerate(ICS_snaps):
        ...
        # Define bins for contour plot
        x_bins = np.linspace(10, 15, 100)
        y_bins = np.linspace(7, 12, 100)
    ...
    for idx, snap in enumerate(ICS_snaps):
        ...
        # Calculate median in bins
        bin_width = 0.25
        bin_edges = np.arange(8, 12, bin_width)
        bin_centers = bin_edges[:-1] + bin_width / 2
    ...
    # Define bins for contour plot
    x_bins = np.linspace(0, 1.25, 100)
    y_bins = np.linspace(-1, 1.0, 100)
    ...
    bin_width = 0.25
    bin_edges = np.arange(0, 1.5, bin_width)
    ...
    for idx, snap in enumerate(ICS_snaps):
        ...
        # Define bins for contour plot
        x_bins = np.linspace(6, 12, 100)
        y_bins = np.linspace(7, 9, 100)
        ...
        # Calculate median in bins
        bin_width = 0.2
        bin_edges = np.arange(6, 12, bin_width)
        bin_centers = bin_edges[:-1] + bin_width / 2
