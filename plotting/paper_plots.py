...
SFR_SimDirs = [
    ...
    {
        'path': './output/testmodels/miniUchuu_FFB/', 
        'label': 'miniUchuu', 
        'color': 'blue', 
        'linestyle': '--',
    ...
    # {
    #     'path': '../../SAGE_BROKEN/sage-model/output/millennium/', 
    #     'label': 'evilSAGE', 
    #     'color': 'red', 
    #     'linestyle': ':',
    #     'BoxSize': 500,  # h-1 Mpc
...
# Process main simulation (SAGE26)
if os.path.exists(main_sim_path):
    logger.info('Processing main simulation (SAGE26)...')
    median_sfr_main, mass_bin_means_main, lookback_times_main, mass_bin_centers = process_simulation_sfr_evolution(
        main_sim_path, 'SAGE26', redshifts, FirstSnap, LastSnap, Main_Hubble_h)
    ...
    for bin_idx, sfr_evolution in median_sfr_main.items():
        ...
        if np.sum(nonzero_mask) > 3:
            ...
            plt.plot(lookback_reversed, np.log10(sfr_reversed + 1e-5), 
                    color=line_color, linewidth=4.0, linestyle='-',
                    alpha=0.8)
...
ax.set_xlabel(r'Lookback Time (Gyr)', fontsize=14)
ax.set_ylabel(r'$\log_{10}$ SFR $(M_{\odot}\ \mathrm{yr}^{-1})$', fontsize=14)
...
if __name__ == '__main__':
    ...
    plt.ylabel(r'$\log_{10} \mathrm{\rho_{SFR}}\ (M_{\odot}\ \mathrm{yr}^{-1}\ \mathrm{Mpc}^{-3})$', fontsize=14)
    plt.xlabel(r'$\mathrm{Redshift}$', fontsize=14)
    ...
    plt.ylabel(r'$\log_{10}\ \rho_{*}\ (M_{\odot}\ \mathrm{Mpc}^{-3})$', fontsize=14)
    plt.xlabel(r'$\mathrm{Redshift}$', fontsize=14)
